	...
	try {
		...
		app.get("/api/courses", async (req, res) => {
			...
			if (filter === "popular") {
				...
				const sortedCourses = courseIds.map((id) => courses.find((course) => course._id.toString() === id));

				res.send(sortedCourses);
			} else if (filter === "recent") {
				const courses = await courseCollection.find({}).sort({ createdAt: -1 }).limit(limitNumber).toArray();
				res.send(courses);
			} else {
				const courses = await courseCollection.find({}).limit(limitNumber).toArray();
				res.send(courses);
			}
		});
		...
		app.get("/api/courses/:id", async (req, res) => {
			...
			try {
				...
				res.status(500).send({ error: true, message: "Server error", details: error.message });
			}
		});
		...
		app.get("/api/my-courses", verifyFirebaseToken, verifyTokenEmail, async (req, res) => {
			...
			const courses = await courseCollection.find(query).sort(sortOption).toArray();
			res.send(courses);
		});

		app.get("/api/edit-course/:id", verifyFirebaseToken, verifyTokenEmail, async (req, res) => {
			const id = req.params.id;
			const email = req.query.email;
		...
		app.patch("/api/edit-course/:id", verifyFirebaseToken, verifyTokenEmail, async (req, res) => {
			const id = req.params.id;
			const update = req.body;
			const filter = { _id: new ObjectId(id), authorEmail: req.decoded.email };

			const result = await courseCollection.updateOne(filter, { $set: update });

			if (result.modifiedCount === 0) {
				return res.status(403).send({ error: true, message: "Unauthorized or no change" });
			}

			res.send(result);
		});

		app.delete("/api/delete-course/:id", verifyFirebaseToken, verifyTokenEmail, async (req, res) => {
			const id = req.params.id;
			const email = req.query.email;
		...
		app.post("/api/enroll", verifyFirebaseToken, async (req, res) => {
			...
			if (!course) {
				return res.status(404).send({ error: true, message: "Course not found" });
			}

			if (course.authorEmail === email) {
				return res.status(403).send({ error: true, message: "You cannot enroll in your own course." });
			}
			...
			if (existing) {
				...
				await courseCollection.updateOne({ _id: new ObjectId(courseId) }, { $inc: { seats: 1 } });
				const updated = await courseCollection.findOne({ _id: new ObjectId(courseId) });
				return res.send({
					success: true,
			...
			if (count >= 3) {
				return res.status(403).send({ error: true, message: "You can enroll in only 3 courses at a time" });
			}
			...
			await enrollmentCollection.insertOne({ email, courseId, enrolledAt: new Date() });
			await courseCollection.updateOne({ _id: new ObjectId(courseId) }, { $inc: { seats: -1 } });
			const updated = await courseCollection.findOne({ _id: new ObjectId(courseId) });
		...
		app.get("/api/my-enrollments", verifyFirebaseToken, verifyTokenEmail, async (req, res) => {
			const email = req.query.email;
			...
			try {
				...
				console.error(err);
				res.status(500).send({ success: false, message: "Failed to load enrolled courses" });
			}
		});
	} finally {
	}
